...
type Scene = { id: string; title: string; start: string; duration: number; narration: string; claimType: ClaimType; visualPrompt: string; motion: string; sound: string; sourceNeed: string };
...
function App() {
  ...
  return <div className='appShell'>
    ...
    <header className='topbar'><div className='brand'>GRIMFORGE <span>XIII</span></div><div className='topActions'><button className='ghost' onClick={()=>setCopilotOpen(true)}>Ask Veyr</button><button className='ghost' onClick={()=>setManualOpen(!manualOpen)}>{manualOpen?'Hide Manual Studio':'Edit manually'}</button></div></header>
    <main>
      ...
      <section className='hero simpleHero'>
        <div className='kicker'>Veyr One-Click Creator</div><h1>Tell Veyr what episode you want.</h1>
        <p>He can build the script, scenes, canon labels, visual plan, voice direction, title, thumbnail and next-episode bridge. You can still open the full studio and change everything yourself.</p>
        <textarea className='masterPrompt' value={prompt} onChange={(e)=>setPrompt(e.target.value)} placeholder='Example: Make a 5-minute episode about why the Imperium needs the war, with a dark analytical tone and one premium 3D shot.'/>
        <div className='creatorQuick'><label>World<select value={world} onChange={(e)=>setWorld(e.target.value)}><option>Warhammer 40K</option><option>Old World / Warhammer Fantasy</option></select></label><label>Length<select value={length} onChange={(e)=>setLength(+e.target.value)}><option value={5}>5 minutes</option><option value={8}>8 minutes</option><option value={12}>12 minutes</option></select></label><button className='primary mega' disabled={!!busy||!prompt.trim()} onClick={createEpisode}>✦ Create Full Episode</button></div>
        <div className='primaryActions'><button className='watchBtn' onClick={()=>{setPreviewIndex(0);setPreviewPlaying(true);document.getElementById('preview')?.scrollIntoView({behavior:'smooth'});}}>▶ Watch Preview</button><button onClick={()=>setManualOpen(true)}>✎ Edit Manually</button><button onClick={saveCloud}>☁ Save to Cloud</button><button onClick={()=>document.getElementById('publish')?.scrollIntoView({behavior:'smooth'})}>↑ Publish</button></div>
      </section>
      {notice&&<div className='notice'>{notice}</div>}

      <section className='productionReadiness'>
      ...
      <section className={cinematicMode?'cinematicForge active':'cinematicForge'}>
      ...
      <section className='episodeSummary'><div><div className='kicker'>Current Episode</div><h2>{episode.title}</h2><p>{episode.thesis}</p></div><div className='summaryBadges'><span>{episode.targetMinutes} min</span><span>Kokoro Local Neural</span><span>{speechProfile}</span><span>{dialogueMode}</span><span>{style}</span></div></section>
      ...
      <section className='previewStudio' id='preview'>
        ...
        <div className='mediaInputs'><label>Load / replace rendered MP4<input type='file' accept='video/mp4,video/*' onChange={(e)=>mp4Input(e.target.files?.[0])}/></label><label>Use your own final narration<input type='file' accept='audio/*' onChange={(e)=>narrationInput(e.target.files?.[0])}/></label>{narrationUrl&&<audio src={narrationUrl} controls/>}</div>
        <p className='fine'>Important: GrimForge's generated preview is an editable directed animatic, not a claim of full AI motion rendering. Cinematic Forge now creates continuity-aware shot direction and can generate the full scene storyboard. A true moving MP4 still requires a connected video-render engine; any real MP4 you load plays here.</p>
      </section>

      <section className='visualChooser' id='visual-style'>
      ...
      {manualOpen&&<section className='manualStudio' id='manual-studio'>
