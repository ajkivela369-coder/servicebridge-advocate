...
type SmartEvidenceMetadata = { documentId: string; providerCandidates: string[]; facilityCandidates: string[]; recordDateCandidates: string[]; documentTypeCandidate: string; issueTags: string[]; confidence: 'low' | 'medium' | 'high'; verificationStatus: 'Unreviewed' | 'Reviewed'; generatedAt: string };
type EvidenceInventoryItem = { id: string; name: string; category: EvidenceCategory; documentType: string; locator: string; candidateDates: string[]; fingerprint: string; versionGroup: string; storageStatus: 'Stored original' | 'Derived reference' | 'Indexed only'; classificationBasis: 'heuristic'; smartMetadata?: SmartEvidenceMetadata | null };
...
function App() {
    ...
    const runCopilot = async (rawPrompt: string, requestedTool?: PluginName) => {
        ...
        setNotice('');
        try {
            const preferredTool = requestedTool ?? tool;
            const selectedTool = plugins[preferredTool] ? preferredTool : 'Record Search';
            const workspaceInstruction = view === 'review'
                ? '[Current workspace: Evidence Auditor. Prioritize source support, contradictions, chronology, provenance, functional reliability, and reviewer-readiness.]'
                : view === 'cloud'
                    ? '[Current workspace: Evidence Cloud. Prioritize finding, organizing, linking, and explaining the loaded evidence without altering original sources.]'
                    : view === 'packet'
                        ? '[Current workspace: Packet Studio. Prioritize verified evidence, source citations, packet structure, preflight blockers, and export readiness.]'
                        : '[Current workspace: Elias. Route internally to the evidence capability needed for the user request without asking the user to choose a specialist bot.]';
            const { data } = await api.post('/api/chat', { question: workspaceInstruction + '\n' + prompt, threadId, tool: selectedTool, speed: workMode });
            setChats((prev) => [...prev, data.message]);
    ...
    const enrichSelectedMetadata = async () => {
        if (!bundleSelectedIds.length) { setNotice('Select one or more Evidence Cloud sources before running smart metadata.'); return; }
        setBusy('Extracting candidate provider, facility, date, type, and issue metadata…');
        setNotice('');
        try {
            const { data } = await api.post('/api/evidence/metadata/enrich', { documentIds: bundleSelectedIds });
            await loadEvidenceInventory();
            setNotice(`Smart metadata generated for ${data.metadata?.length ?? 0} source${data.metadata?.length === 1 ? '' : 's'}. These are review candidates, not source facts.`);
        } catch (err) {
            setNotice(errorMessage(err, 'Smart metadata could not be generated.'));
        } finally { setBusy(''); }
    };

    const setMetadataReviewed = async (documentId: string, reviewed: boolean) => {
        try {
            await api.post('/api/evidence/metadata/review', { documentId, status: reviewed ? 'Reviewed' : 'Unreviewed' });
            await loadEvidenceInventory();
            setNotice(reviewed ? 'Metadata marked reviewed. This records human review; it does not convert AI candidates into original-source facts.' : 'Metadata returned to Unreviewed.');
        } catch (err) {
            setNotice(errorMessage(err, 'Metadata review status could not be changed.'));
        }
    };
    ...
    if (!user) {
        return <main className='public-site'>
            <nav className='public-nav'><div className='brand'><div className='brand-mark small'><Bot size={22} /></div><div><strong>Elias</strong><span>+ Evidence Auditor</span></div></div><div><a href='#how-it-works'>How it works</a><a href='#capabilities'>Capabilities</a><button className='primary' onClick={signIn}><LogIn size={16} /> Sign in</button></div></nav>
            <section className='public-hero'><div className='public-hero-copy'><p className='eyebrow'>PATIENT-CONTROLLED EVIDENCE INTELLIGENCE</p><h1>Every conclusion should lead back to the record.</h1><p>Elias organizes the case. Evidence Auditor asks the harder question: what actually supports this statement, where is it in the source, what conflicts with it, and what is still missing?</p><div className='public-actions'><button className='primary large' onClick={signIn}><Sparkles size={18} /> Start an evidence workspace</button><a className='secondary' href='#capabilities'>Explore the system</a></div><div className='public-proof'><span><ShieldCheck size={15} /> Original-source control</span><span><FileSearch size={15} /> Claim → Source Trace</span><span><Cloud size={15} /> Shared Evidence Cloud</span></div></div><div className='public-hero-panel'><span className='result-kicker'>EVIDENCE TRACE</span><h2>Source → locator → evidence → classification → interpretation → statement</h2><div className='public-trace'><div><b>01</b><span>Source record</span></div><div><b>02</b><span>Page / locator</span></div><div><b>03</b><span>Evidence type</span></div><div><b>04</b><span>Human-reviewed output</span></div></div><small>Broken or ambiguous links become warnings—not silent guesses.</small></div></section>
            <section id='capabilities' className='public-section'><div className='public-section-head'><p className='eyebrow'>ONE APP · ONE ELIAS ENGINE</p><h2>Evidence Auditor is Elias’s flagship feature.</h2><p>Search, citation checking, health-record explanation, neurologic/function review, document building, and packet creation are capabilities Elias invokes automatically—not separate bots the user has to understand.</p></div><div className='public-grid'><article><Cloud size={22} /><h3>Evidence Cloud</h3><p>Private case storage, indexing, transparent metadata, duplicate/version review, saved bundles, search, and persistent chronology.</p></article><article><FileSearch size={22} /><h3>Evidence Auditor</h3><p>Support, contradictions, gaps, provenance, Claim → Source Trace, quote verification, and reviewer-readiness.</p></article><article><Brain size={22} /><h3>Health + Neuro Lens</h3><p>When the task is medical, neurologic, or functional, Elias shifts into a stricter health-evidence lens automatically—without changing assistants or losing the case context.</p></article><article><FileImage size={22} /><h3>Visual Evidence Labs</h3><p>Source-stamped page captures, de-identified illustrations, provenance metadata, and an approved derived-visual library.</p></article><article><Scale size={22} /><h3>Authority Verification</h3><p>Program-specific VA, SSA, New Hampshire, and medical-function references with authority type, live reachability, and check time.</p></article><article><WandSparkles size={22} /><h3>Packet Studio</h3><p>Polished source-backed PDFs, selected visual pages, exact file-size checks, all-pages-rendered validation, and packet-only visual reduction.</p></article></div></section>
            <section id='how-it-works' className='public-section public-flow'><div className='public-section-head'><p className='eyebrow'>HOW IT WORKS</p><h2>Upload once. Trace everything.</h2></div><div className='public-steps'><div><b>1</b><strong>Upload</strong><span>Bring in PDFs, scans, images, email evidence, and supported records.</span></div><div><b>2</b><strong>Organize</strong><span>Build the Evidence Cloud, bundles, metadata, and timeline.</span></div><div><b>3</b><strong>Audit</strong><span>Trace claims, verify quotes, reconcile conflicts, and identify gaps.</span></div><div><b>4</b><strong>Build</strong><span>Create approved visuals and a structured filing draft.</span></div><div><b>5</b><strong>Verify</strong><span>Check provenance, authorities, exact PDF size, and every rendered page.</span></div></div></section>
            <section className='public-section public-specialists'><div className='public-section-head'><p className='eyebrow'>FLOATING ELIAS</p><h2>One assistant. The tools stay behind the curtain.</h2></div><div className='chips'><span>Find</span><span>Compare</span><span>Explain</span><span>Organize</span><span>Link</span><span>Draft</span><span>Build</span><span>Verify</span><span>Export</span></div><p>Elias chooses the evidence capability from the current workspace and request; the user does not have to pick a bot.</p></section>
            <section className='public-boundary'><ShieldCheck size={26} /><div><h2>The record remains the record.</h2><p>AI summaries, metadata, timelines, illustrations, and filing drafts are separate derived layers. Human verification remains required. The site does not claim HIPAA compliance unless the deployed infrastructure, controls, data flows, and applicable contracts are actually verified.</p></div><button className='primary large' onClick={signIn}>Open Elias + Evidence Auditor</button></section>
            {notice && <div className='notice public-notice'>{notice}</div>}
        </main>;
    }
    ...
    return <div className='shell'>
        ...
        <main className='main'>
            ...
            {view === 'cloud' && <section className='workspace'>
                ...
                <section className='vault-panel'>
                    <div className='vault-head'><div><span className='result-kicker'>SMART METADATA</span><h2>Reviewable metadata layer</h2><p>Select case sources below, then extract candidate provider/facility names, record dates, document type, and issue tags. Candidates stay separate from source evidence and remain Unreviewed until a person checks them.</p></div><button className='primary' disabled={!bundleSelectedIds.length || !!busy} onClick={() => void enrichSelectedMetadata()}><Sparkles size={15} /> Enrich selected ({bundleSelectedIds.length})</button></div>
                </section>
                ...
                <h2 className='section-title'>Cloud source inventory</h2><div className='doc-list'>{filteredDocuments.map((document) => { const meta = inventoryById.get(document.id); return <article key={document.id} className='doc-card'><div>{document.sourceMode === 'ocr' ? <FileImage size={19} /> : document.sourceMode === 'email' ? <Mail size={19} /> : ['web', 'search'].includes(document.sourceMode || '') ? <Globe2 size={19} /> : document.sourceMode === 'media' ? <ImagePlus size={19} /> : <FolderOpen size={19} />}<div><strong>{document.name}</strong><span>{meta?.documentType || categorizeDocument(document)} · {meta?.locator || `${document.pageCount} indexed page${document.pageCount === 1 ? '' : 's'}`} · {Math.round(document.charCount / 1000)}k characters</span>{document.sourceUrl && <a className='source-link' href={document.sourceUrl} target='_blank' rel='noreferrer'>{document.sourceUrl}</a>}</div></div><p>{document.excerpt}</p><div className='chips'><span>{meta?.category || categorizeDocument(document)}</span><span>{meta?.storageStatus || document.sourceMode || 'indexed'}</span>{meta?.candidateDates?.slice(0, 2).map((date) => <span key={`${document.id}-${date}`}>{date}</span>)}{meta && <span>HEURISTIC METADATA</span>}{meta?.smartMetadata && <span>{meta.smartMetadata.verificationStatus.toUpperCase()} · {meta.smartMetadata.confidence.toUpperCase()} CONFIDENCE</span>}</div>{meta?.smartMetadata && <div className='argument-support'><strong>AI CANDIDATE METADATA · {meta.smartMetadata.verificationStatus}</strong>{meta.smartMetadata.providerCandidates.length > 0 && <p>Provider candidate: {meta.smartMetadata.providerCandidates.join(' · ')}</p>}{meta.smartMetadata.facilityCandidates.length > 0 && <p>Facility candidate: {meta.smartMetadata.facilityCandidates.join(' · ')}</p>}{meta.smartMetadata.recordDateCandidates.length > 0 && <p>Date candidate: {meta.smartMetadata.recordDateCandidates.join(' · ')}</p>}{meta.smartMetadata.documentTypeCandidate && <p>Document type candidate: {meta.smartMetadata.documentTypeCandidate}</p>}{meta.smartMetadata.issueTags.length > 0 && <p>Issue tags: {meta.smartMetadata.issueTags.join(' · ')}</p>}<small>Review status records that a human looked at these candidates; it does not convert them into source evidence.</small><button className='secondary tiny' onClick={() => void setMetadataReviewed(document.id, meta.smartMetadata?.verificationStatus !== 'Reviewed')}>{meta.smartMetadata.verificationStatus === 'Reviewed' ? 'Return to Unreviewed' : 'Mark reviewed'}</button></div>}<label className='save-check'><input type='checkbox' checked={bundleSelectedIds.includes(document.id)} onChange={() => toggleBundleSource(document.id)} /> Include in evidence bundle</label>{document.contentType === 'application/pdf' && <button className='secondary tiny' onClick={() => void openSourcePageLab(document)}><FileImage size={14} /> Page Lab</button>}<button onClick={() => void removeDoc(document.id)}><Trash2 size={15} /> Delete</button></article>; })}</div>
        ...
        <button className={copilotOpen ? 'elias-fab open nonprint' : 'elias-fab nonprint'} onClick={() => setCopilotOpen(!copilotOpen)}><span><Bot size={19} /></span><div><strong>Elias</strong><small>{copilotOpen ? 'Close' : 'Context-aware evidence engine'}</small></div></button>
        {copilotOpen && <aside className='elias-copilot nonprint'>
            <div className='copilot-head'><div><p className='eyebrow'>One engine · context-aware tools</p><h3>Elias</h3><span>{documents.length} indexed source{documents.length === 1 ? '' : 's'} · {workMode} mode · {view === 'review' ? 'Evidence Auditor context' : view === 'cloud' ? 'Evidence Cloud context' : view === 'packet' ? 'Packet Studio context' : 'case-wide context'}</span></div><button onClick={() => setCopilotOpen(false)}><X size={18} /></button></div>
