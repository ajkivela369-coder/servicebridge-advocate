...
function App() {
    ...
    if (!user) {
        return <main className='login-wrap'><section className='login-card'><div className='brand-mark'><Bot size={32} /></div><p className='eyebrow'>Evidence Auditor Pro</p><h1>Meet Elias.</h1><p>A private, source-grounded evidence workspace with a streamlined Simple mode and the full Pro evidence-review workstation.</p><button className='primary large' onClick={signIn}><LogIn size={18} /> Sign in to Elias</button><p className='fine'>Your workspace is private to your signed-in account. Elias organizes evidence; he does not make legal or medical determinations.</p>{notice && <div className='notice'>{notice}</div>}</section></main>;
    }
    ...
    return <div className='shell'>
        ...
        <main className='main'>
            <header className='topbar nonprint'><div><PanelLeft size={17} /><span>{view === 'chat' ? 'Elias' : view === 'review' ? 'Case Review' : 'Packet Studio'}</span></div><div className='top-controls'><span className={`speed-pill ${workMode.toLowerCase()}`}><Gauge size={14} /> {workMode}</span><div className='privacy'><ShieldCheck size={16} /> Private workspace</div><div className='elias-mode-switch pro-top-mode-switch'><button onClick={() => setAppMode('simple')}>Simple</button><button className='active'>Pro</button></div></div></header>
