...
function App() {
    ...
    if (!user) {
        return <main className='public-site'>
            ...
            <section id='capabilities' className='public-section'><div className='public-section-head'><p className='eyebrow'>ONE APP · SPECIALIZED INTELLIGENCE</p><h2>Elias is the shell. Evidence Auditor is the flagship audit engine.</h2><p>The specialists share one case record instead of asking the user to upload the same evidence into separate bots.</p></div><div className='public-grid'><article><Cloud size={22} /><h3>Evidence Cloud</h3><p>Private case storage, indexing, transparent metadata, duplicate/version review, saved bundles, search, and persistent chronology.</p></article><article><FileSearch size={22} /><h3>Evidence Auditor</h3><p>Support, contradictions, gaps, provenance, Claim → Source Trace, quote verification, and reviewer-readiness.</p></article><article><Brain size={22} /><h3>NeuroEval + HealthQA</h3><p>Specialist neurological/functional review and patient-friendly record explanation without promoting inference into diagnosis.</p></article><article><FileImage size={22} /><h3>Visual Evidence Labs</h3><p>Source-stamped page captures, de-identified illustrations, provenance metadata, and an approved derived-visual library.</p></article><article><Scale size={22} /><h3>Authority Verification</h3><p>Program-specific VA, SSA, New Hampshire, and medical-function references with authority type, live reachability, and check time.</p></article><article><WandSparkles size={22} /><h3>Packet Studio</h3><p>Polished source-backed PDFs, selected visual pages, exact file-size checks, all-pages-rendered validation, and packet-only visual reduction.</p></article></div></section>
            ...
            <section className='public-section public-specialists'><div className='public-section-head'><p className='eyebrow'>FLOATING COPILOT</p><h2>One assistant interface, multiple specialist modes.</h2></div><div className='chips'><span>Elias</span><span>Evidence Auditor</span><span>NeuroEval</span><span>HealthQA</span><span>Packet Builder</span><span>Citation Auditor</span><span>Document Copilot</span></div><p>Find → Compare → Explain → Organize → Link → Draft → Build → Verify → Export.</p></section>
