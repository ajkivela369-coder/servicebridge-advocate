    ...
    return <div className='shell'>
        ...
        <main className='main'>
            ...
            {view === 'review' && <section className='workspace'>
                ...
                <div className='research-grid nonprint'>
                    <article className='research-panel'>
                        ...
                        <select value={lawIssue} onChange={(e) => setLawIssue(e.target.value)}><option>Service connection</option><option>Secondary / aggravation</option><option>Increased rating</option><option>TDIU</option><option>Mental health rating</option><option>Neurologic / migraines</option><option>Musculoskeletal rating</option><option>New Hampshire APTD eligibility</option></select>
            ...
            {view === 'packet' && <section className='workspace'>
                <div className='page-head nonprint'><div><p className='eyebrow'>Submission builder</p><h1>Packet Studio</h1><p>Draft the narrative here, then assemble selected source PDF pages with the local packet CLI. Review every page and current law before submission.</p></div><div className='packet-head-actions'><button className='secondary' disabled={copilotBusy || !!busy} onClick={() => void runCopilot(quickEliasActions[5].prompt, 'Packet Assurance')}><ShieldCheck size={17} /> One-click preflight</button><button className='primary' onClick={() => void generatePacket()}><WandSparkles size={17} /> Generate draft · {workMode}</button></div></div>
                ...
                <div className='connector-note nonprint'><ShieldCheck size={17} /><div><strong>Submission readiness</strong><span>Attach the original clinical pages, labeled injury illustrations and dated screenshots; verify page provenance, clickable links, current VA and state law, and the recipient size limit. Print / Save PDF exports this text draft only. Use the local packet manifest to preserve source pages and links.</span></div></div>
