    ...
    return <div className='shell'>
        ...
        <main className='main'>
            ...
            {view === 'cloud' && <section className='workspace'>
                <input ref={fileRef} className='hidden' multiple type='file' accept='.pdf,.txt,.md,.eml,.mbox,.png,.jpg,.jpeg,.webp,application/pdf,text/plain,text/markdown,message/rfc822,application/mbox,image/png,image/jpeg,image/webp' onChange={(e) => void upload(e.target.files)} /><div className='page-head'><div><p className='eyebrow'>Patient evidence cloud</p><h1>Evidence Cloud</h1><p>One persistent source library for Elias, Evidence Auditor, NeuroEval, HealthQA, and every packet workflow. Upload once, then search, organize, and reference the same case evidence everywhere.</p></div><button className='primary' onClick={() => fileRef.current?.click()}><Paperclip size={17} /> Add evidence</button></div>
