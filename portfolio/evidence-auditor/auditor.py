...
FAVORABLE_TERMS = {"supports", "consistent with", "at least as likely", "aggravated", "worsened", "functional limitation", "unable", "credible", "objective", "documented", "service", "duty", "onset", "continuity", "nexus", "positive opinion"}
UNFAVORABLE_TERMS = {"less likely", "not related", "no evidence", "normal examination", "resolved", "preexisting without aggravation", "negative opinion", "inconsistent", "denied"}
SOURCE_HINTS = {
...
ISSUE_HINTS = {
    ...
    "aggravation": ["aggravated", "worsened", "preexisting", "flare"],
...
MISSING_RECORD_HINTS = ["unavailable", "missing", "not found", "incomplete", "not obtained", "unable to locate", "records were unavailable"]
...
def classify_sentence(sentence: str) -> EvidenceItem:
    ...
    rationale = []
    if favorable: rationale.append("Favorable indicators: " + ", ".join(favorable[:4]))
    if unfavorable: rationale.append("Unfavorable indicators: " + ", ".join(unfavorable[:4]))
    ...
    return EvidenceItem(sentence, stance, infer_source_type(sentence), round(confidence, 2), infer_issues(sentence), rationale)
...
def audit_text(text: str) -> dict:
    ...
    missing_records = [i.text for i in items if any(h in i.text.lower() for h in MISSING_RECORD_HINTS)]
    contradictions = []
    ...
    for fav in favorable_items:
        for unfav in unfavorable_items:
            ...
            if shared_issues:
                contradictions.append({"type": "cross_stance_issue_tension", "issues": shared_issues, "favorable_passage": fav.text, "unfavorable_passage": unfav.text, "note": "Opposing passages address the same issue; human reconciliation is recommended."})

    matrix = [{"stance": i.stance, "source_type": i.source_type, "issues": ", ".join(i.issues), "confidence": i.confidence, "passage": i.text} for i in items]
    return {
        "summary": counts,
